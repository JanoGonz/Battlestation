    ...
    with app.app_context():
        ...
        if len(members) > tournament.min_participants and len(members) > 1:
            if tournament.type == 0:
                for x, y in itertools.zip_longest(members[0::2], members[1::2], fillvalue=0):
                    if y == 0:
                        ...
                        t_alert = TournamentAlert(tournament_id = t, user_id = x.id, round = 1, message =tournament.name + " - La ronda 1 ha comenzado")
                        t_alert_2 = TournamentAlert(tournament_id = t, user_id = y.id, round = 1, message =tournament.name + " - La ronda 1 ha comenzado")
                        db.session.add(t_alert)
                        db.session.add(t_alert_2)
                        x.add_notification('tournament_alert_count', 0, x.new_alerts())
                        y.add_notification('tournament_alert_count', 0, y.new_alerts())
                        db.session.commit()
                        db.session.add(pairing)
                        db.session.commit()
            #END CREATE PAIRINGS
            #START LEAGUE
                ...
                for m in members:
                        t_alert = TournamentAlert(tournament_id = t, user_id = m.id, round = 1, message =tournament.name + " - La ronda 1 ha comenzado")
                        db.session.add(t_alert)
...
def schedule_round(t, n_round):
	...
	#START CREATE PAIRINGS
        ...
        if tournament.type== 0:
            for x, y in itertools.zip_longest(members[0::2], members[1::2], fillvalue=0):
                if y == 0:
                    ...
                    t_alert = TournamentAlert(tournament_id = t, user_id = x.id, round = 1, message =tournament.name + " - La ronda " + str(n_round) + " ha comenzado")
                    t_alert_2 = TournamentAlert(tournament_id = t, user_id = y.id, round = 1, message =tournament.name + " - La ronda " + str(n_round) + " ha comenzado")
                    db.session.add(t_alert)
                    db.session.add(t_alert_2)
                    db.session.commit()
                    x.add_notification('tournament_alert_count', 0, x.new_alerts())
        ...
        #No manda alerta si no hay pairing
                ...
                members_se = User.query.join(Membership, Membership.user_id==User.id).filter(Membership.tournament_id==t).filter(Membership.banned==False).filter(Membership.loses < 2).order_by(func.random()).all()
                if len(members_se) > 1:
                    for x, y in itertools.zip_longest(members_se[0::2], members_se[1::2], fillvalue=0):
                        if y == 0:
                            pairing = Pairing(id_tournament = t, user_1 = x.id, user_2 = 0, result_1 = 1, result_2 = 0, round = n_round)
